...
class Bridge(object):
    ...
    def privaterKey(self, _PRIVATE_KEY, _PRIVATE_KEY_PWD):
        ...
        try:
            ...
            pkey = paramiko.RSAKey.from_private_key((_PRIVATE_KEY), _PRIVATE_KEY_PWD)
        except paramiko.SSHException:
            #pkey = paramiko.DSSKey.from_private_key(StringIO(_PRIVATE_KEY), _PRIVATE_KEY_PWD)
            pkey = paramiko.DSSKey.from_private_key((_PRIVATE_KEY), _PRIVATE_KEY_PWD)
        return pkey

    def isPassword(self, data):
        if 'ispwd' in data.keys():
            if data['ispwd']:
                return True
            else:
                return False
        ...
        try:
            ...
            if self.isPassword(data):
                ...
                self.ssh.connect(
                    ...
                    port=int(data["port"]),
                    username=data["username"],
                    pkey=self.privaterKey(self.parse_pkey((data["secret"].encode("utf-8"))), None),
                    timeout=60.0
                )
